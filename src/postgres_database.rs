...
pub async fn init_postgres_client() -> Result<Client, Box<dyn std::error::Error + Send + Sync>> {
...
pub async fn get_last_height_postgres(
    client: &Arc<Client>,
...
pub async fn insert_rows_postgres(
    client: &Arc<Client>,
    rows: &[EventRow],
...
async fn insert_rows_internal_postgres(
    client: &Arc<Client>,
    rows: &[EventRow],
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    if rows.is_empty() {
...
pub async fn check_postgres_connection(
    client: &Arc<Client>,
